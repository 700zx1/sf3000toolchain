...
#define APLIST_NUM          256
#define WIFI_MAX_SSID_LEN   64
#define WIFI_MAX_PWD_LEN    64
#define WIFI_MAX_REPLY_LEN  (32 * 1024)
...
enum
{
    ...
    HCCAST_WIFI_ERR_DEVICE_BUSY             = -0x7,
    ...
    HCCAST_WIFI_ERR_CMD_IWPRIV_NO_DEVICE    = -0x113,
...
{
    char wifi_ifname[64];   // wifi interface name, Reserved
    char p2p_ifname[64];    // p2p interface name, Reserved
    char device_name[64];   // p2p device name
    short listen_channel;   // p2p listen channel
    short oper_channel;     // p2p oper channel
    hccast_wifi_mgr_p2p_state_callback func_update_state;   // p2p state handle cb; hccast default set
    hccast_wifi_mgr_p2p_set_resolution func_set_resolution; // Reserved
 ...
 *
*/
int hccast_wifi_mgr_get_current_freq_p2p();
 ...
 */
int hccast_wifi_mgr_op_abort(void);
 ...
 */
int hccast_wifi_mgr_op_reset(void);
...
int hccast_wifi_mgr_get_sta_ifname(char *ifnmae, int len);
...
/**
 * It Set the bitmap of the disabled oper channel list during P2P negotiation,.
 *
 * @param list op ch bitmap.The available bits for the op ch list (1-13)
 *
 * @return 0
 */
...
typedef enum 
{
    HCCAST_WIFI_PM_MODE_NONE,
    HCCAST_WIFI_PM_MODE_HW,
    HCCAST_WIFI_PM_MODE_PS,
} hccast_wifi_pm_e;
