...
typedef enum IMG_TRANSCODE_MODE {
	/*
	 * FIT: 长宽保持比例，全部缩到边框内
	 * CENTER: 长宽裁剪到边框尺寸。
	 */
	IMG_TRANSCODE_MODE_FIT = 0,
	IMG_TRANSCODE_MODE_CENTER,
} img_transcode_mode_e;

typedef enum IMG_TRANSCODE_FORMAT {
	IMG_TRANSCODE_JPG,
	IMG_TRANSCODE_BGRA,    /* packed BGRA 8:8:8:8, 32bpp, BGRABGRA... */
	IMG_TRANSCODE_YUV420P, /* planar YUV 4:2:0, 12bpp, (1 Cr & Cb sample per 2x2 Y samples), IMG_TRANSCODE_YUV420P is used for internal only */
	IMG_TRANSCODE_YUV_Y_ONLY,
} img_transcode_format_e;
...
struct video_transcode_config {
	...
	/* if (transcode_mode != -1), then dst_width & dst_height will be used */
	int16_t transcode_width;
	int16_t transcode_height;
	img_transcode_mode_e transcode_mode;
	img_transcode_format_e transcode_format;
};
